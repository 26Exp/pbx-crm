...
const ProductsPage = () => {
  ...
  // Removed selectedRows state as bulk functionality is no longer needed
  ...
  // Form state for adding/editing products - simplified to only include name
  ...
  // Only fetch products when these dependencies change, not on every render
  useEffect(() => {
    console.log("Fetching products with:", { currentPage, itemsPerPage, searchTerm });
    fetchProducts();
  ...
  // Removed fetchCategories as it's no longer needed
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      // Use our API service to fetch products
      ...
      const dummyData = generateDummyProducts(36); // Match API example count
  ...
  // Function to generate dummy product data - simplified to match API
  const generateDummyProducts = (count) => {
    // Generate more dummy products to demonstrate pagination better
    return Array.from({ length: count }, (_, i) => ({
      id: i + 1,
      name: `Produs ${i + 1}${i % 5 === 0 ? ' (Special)' : ''}`
    }));
  };

  // Removed row selection handlers as bulk functionality is no longer needed
  ...
  const validateForm = () => {
    ...
    if (!formData.name.trim()) {
      errors.push('Denumirea produsului este obligatorie');
    }
  ...
  // Removed bulk delete handler as this functionality is no longer needed
  ...
  const handleSubmitAdd = async (e) => {
    ...
    try {
      ...
      // Call API to create product - simplified to only include name
      ...
      setFormErrors([error.message || 'An error occurred while creating the product']);
      setLoading(false);
  ...
  const handleSubmitEdit = async (e) => {
    ...
    try {
      ...
      // Call API to update product - simplified to only include name
      ...
      setFormErrors([error.message || 'An error occurred while updating the product']);
      setLoading(false);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      await apiService.products.delete(editingProduct.id);
  ...
  const AddProductModal = () => (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
        <h2 className="text-xl font-semibold mb-4">Adăugare Produs Nou</h2>
  ...
  const EditProductModal = () => (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
        <h2 className="text-xl font-semibold mb-4">Editare Produs</h2>
  ...
  const DeleteConfirmationModal = () => (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
        ...
        <p className="mb-6">
          Sunteți sigur că doriți să ștergeți produsul "{editingProduct?.name}"? Această acțiune este ireversibilă.
        </p>
        ...
        <div className="flex justify-end space-x-3">
          ...
          <button
            ...
            type="button"
          >
            {loading ? 'Se șterge...' : 'Șterge Produsul'}
          </button>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="p-4">
      <h1 className="text-2xl font-bold mb-6">Lista Produse</h1>
      ...
      <div className="bg-white shadow-md rounded-lg overflow-hidden">
        ...
        {loading ? (
          <div className="p-8 text-center">
            ...
            <p className="mt-2 text-gray-500">Încărcare produse...</p>
          </div>
        ) : products.length === 0 ? (
          <div className="p-8 text-center text-gray-500">
            {searchTerm ? 'Nu s-au găsit produse pentru căutarea curentă.' : 'Nu există produse. Adăugați un produs nou.'}
          </div>
        ) : (
          <>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {products.map((product) => (
                    <tr key={product.id} className="hover:bg-gray-50">
                      {columns.map((column) => (
                        <td key={`${product.id}-${column.key}`} className="px-6 py-4 whitespace-nowrap">
