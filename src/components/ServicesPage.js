...
const ServicesPage = () => {
  ...
  // Form state for adding/editing services - simplified to only include name
  ...
  // Only fetch services when these dependencies change, not on every render
  useEffect(() => {
    console.log("Fetching services with:", { currentPage, itemsPerPage, searchTerm });
    fetchServices();
  ...
  const fetchServices = async () => {
    ...
    try {
      ...
      // Use our API service to fetch services
      ...
      const dummyData = generateDummyServices(36); // Match API example count
  ...
  // Function to generate dummy service data - simplified to match API
  const generateDummyServices = (count) => {
    // Generate more dummy services to demonstrate pagination better
    return Array.from({ length: count }, (_, i) => ({
      id: i + 1,
      name: `Serviciu ${i + 1}${i % 5 === 0 ? ' (Special)' : ''}`
    }));
  };
  ...
  const validateForm = () => {
    ...
    if (!formData.name.trim()) {
      errors.push('Denumirea serviciului este obligatorie');
    }
  ...
  const handleDeleteClick = (service) => {
    ...
    if (!canDelete) {
      console.warn('User does not have permission to delete services');
      return;
    }
  ...
  const handleSubmitAdd = async (e) => {
    ...
    try {
      ...
      // Call API to create service - simplified to only include name
      ...
      setFormErrors([error.message || 'An error occurred while creating the service']);
      setLoading(false);
  ...
  const handleSubmitEdit = async (e) => {
    ...
    try {
      ...
      // Call API to update service - simplified to only include name
      await apiService.services.update(editingService.id, formData);
      ...
      setFormErrors([error.message || 'An error occurred while updating the service']);
      setLoading(false);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      await apiService.services.delete(editingService.id);
      
      // Close the modal first to give immediate visual feedback
      setIsDeleteModalOpen(false);
      
      // Then refresh the services list
      await fetchServices();
    } catch (error) {
  ...
  const AddServiceModal = () => (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
        <h2 className="text-xl font-semibold mb-4">Adăugare Serviciu Nou</h2>
  ...
  const EditServiceModal = () => (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
        <h2 className="text-xl font-semibold mb-4">Editare Serviciu</h2>
  ...
  const DeleteConfirmationModal = () => (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
        ...
        <p className="mb-6">
          Sunteți sigur că doriți să ștergeți serviciul "{editingService?.name}"? Această acțiune este ireversibilă.
        </p>
        ...
        <div className="flex justify-end space-x-3">
          ...
          <button
            ...
            type="button"
          >
            {loading ? 'Se șterge...' : 'Șterge Serviciul'}
          </button>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="p-4">
      <h1 className="text-2xl font-bold mb-6">Lista Servicii</h1>
      ...
      <div className="bg-white shadow-md rounded-lg overflow-hidden">
        ...
        {loading ? (
          <div className="p-8 text-center">
            ...
            <p className="mt-2 text-gray-500">Încărcare servicii...</p>
          </div>
        ) : services.length === 0 ? (
          <div className="p-8 text-center text-gray-500">
            {searchTerm ? 'Nu s-au găsit servicii pentru căutarea curentă.' : 'Nu există servicii. Adăugați un serviciu nou.'}
          </div>
        ) : (
          <>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {services.map((service) => (
                    <tr key={service.id} className="hover:bg-gray-50">
                      {columns.map((column) => (
                        <td key={`${service.id}-${column.key}`} className="px-6 py-4 whitespace-nowrap">
                          {column.render ? column.render(service) : service[column.key]}
                        </td>
                      ))}
                    </tr>
                  ))}
      ...
      {/* Modals */}
      {isAddModalOpen && <AddServiceModal />}
      {isEditModalOpen && <EditServiceModal />}
